...
namespace GeneGenie.Gedcom.Tests.EncoderTests
{
	...
	public class GedcomAnsiEncoderTest
	{
		...
		[InlineData("/Undefined/", 141, "")]
		[InlineData("/Latin Capital Letter Z With Caron/", 142, "Ž")]
		[InlineData("/Undefined/", 143, "")]
		[InlineData("/Undefined/", 144, "")]
		...
		public void Characters_can_be_translated_to_unicode(string childName, int rawAnsiValue,  string expectedUnicode)
        {
			...
			// TODO: Add all of above child names and birth places to ANSI test file.
			// TODO: Ansi reader is not parsing values properly, despite the correct code page.
			// TODO: Add ability to check rawAnsiValue against data read in (may have to surface LineValue).
			//Assert.Equal(rawAnsiValue, child.Birth.Place.Name);
			Assert.Equal(expectedUnicode, child.Birth.Place.Name);
        }
    }
}
